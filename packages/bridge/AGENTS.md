...
- `docs/10-adr-git-stamp.md` — ADR 10: git identity stamps — bridge-bundled extension records HEAD commit/branch transitions as `custom` entries at prompt/turn_end boundaries; shared payload in `core`, fold in `viewmodel`, chip in web.
...
- `src/core/git-stamp.ts` — pure ADR 10 stamp contract: `GIT_STAMP_CUSTOM_TYPE`, `GitStampData`/`GitIdentity`, payload parsing/validation (`parseGitStampData`, `parseGitStampEntry` — accepts both pi `type` and bridge `kind` entry flavors), `parseGitIdentity` (raw git plumbing output), `sameGitIdentity` (transition key)
