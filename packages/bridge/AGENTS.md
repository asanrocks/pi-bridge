...
- `docs/11-adr-projects.md` — ADR 11 (Proposed): projects and sessions — the client-facing domain is `Project` + `Session` only; `Project` becomes first-class (id = validated last path segment, unique, startup-rejected on duplicate ids and session-storage collisions); a session is addressed by its jsonl **file stem** (`<timestamp>_<sessionId>`, direct path mapping) while `sessionId` stays the durable cache key; the stem is allocated at creation (before the first flush), so unflushed sessions are addressable; "active" is session metadata backed by an internal, never-on-the-wire activation (one activation per active session, GC'd when idle) and `listActiveSessions()` returns the same `SessionInfo` shape; `(projectId, stem)` addressability via an `openSession` resolve-or-create verb (no cross-session rebind or reuse); URLs (`/launcher`, `/chat/$proj` as the project launcher, `/chat/$proj/$stem`) are consequences of the model.
