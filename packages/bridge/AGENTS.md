...
- `docs/11-adr-projects.md` — ADR 11 (Proposed): projects, sessions, instances — `Project` becomes first-class (id = validated last path segment, unique, startup-rejected on duplicates/archive-key collisions); a session is addressed by its jsonl **file stem** (`<timestamp>_<sessionId>`, direct path mapping) while `sessionId` stays the durable id (names are metadata); `Instance` is demoted to an ephemeral activation (one session at a time, ≤1 live instance per session, guarded in the daemon); `(projectId, stem)` addressability via an `openSession` resolve-or-activate verb; URLs (`/launcher`, `/chat/$proj/$stem`) and the project-folder sidebar are consequences of the model.
