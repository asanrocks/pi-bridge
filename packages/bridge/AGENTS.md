...
- `src/host/manager.ts` — `createManager(options)`: injectable factory (test seam); owns one pi runtime + canonical `Document`; exposes typed session verbs (`prompt`, `abort`, `discardSteer`, `setModel`, `setThinkingLevel`, `renameSession`, `navigate`, `switchSession(sessionPath, cursor?, initiator?)`, `newSession`) and `onPatch`/`onReplace`/`onExit` callbacks plus `ConnectionHandle`-based `addConnection(handle, cursor?)`/`removeConnection(handle)` (initial sync at attach and rebind: cursor delta to the initiating Connection, replace to others — ADR 09)
- `src/host/connection.ts` — `Connection`: owns one WebSocket, attached to one Manager (or none); `attach(mgr, id, cursor?)`/`detach()`; demuxes incoming RPC frames (routes session verbs → (attached) Manager, routing verbs → Daemon, daemon verbs → Daemon, `pull` → self); filters patches against per-connection subscriptions (cleared on every initial-sync frame — attach and rebind); routes outgoing through a `CompactCodec`; `sendReply` goes through `send` so replies reset codec state too
