...
async function createVerbHarness(useFixture = true): Promise<VerbHarness> {
	...
	const settingsManager = SettingsManager.inMemory({
		...
		// The relay test asserts byte-exact image pass-through. Upstream's
		// prompt path now normalizes images for the provider (resize/convert),
		// which re-encodes or drops the fake fixture image; disable it so the
