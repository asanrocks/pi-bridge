 ...
 * system-prompt diff entry at turn start; it is not an in-flight user/assistant
 * provisional, so the pairing check ignores it. */
function committedNonSystemIds(doc: Document): string[] {
	return committedOf(doc)
...
describe("initial sync: attach", () => {
	...
	it("mid-turn attach includes provisional skeletons and only doc-held committed entries", async () => {
		...
		const bootstrapIds = new Set(committedNonSystemIds(firstReplace.document));
		...
		const frameIds = new Set(committedNonSystemIds(doc));
